...
/// TaskGraph 시각화 (ADR-049 Phase 5).
///
/// 사이드바 또는 inspector에 표시되는 mini-map. 각 task를 row로 표시:
/// - status 아이콘 (pending/running/completed/failed)
/// - assigned agent badge
/// - title + description preview
/// - 의존성 표시 (depth or arrow)
public struct TaskGraphMiniMap: View {
    ...
    private var header: some View {
        HStack(spacing: 6) {
            ...
            HelpHint(
                "Harness가 추적하는 task graph. /decompose 명령으로 큰 task를 LLM이 자동 분해할 수 있어요. 의존성이 있으면 들여쓰기로 표시.",
                title: "Task Graph",
                placement: .bottom
            )
            Spacer()
